...
(async () => {
  ...
  const token = encrypt(tokenize("internal_counter/my_counter"))
  const data = {
    "key":config.key,
  ...
  try {
    const response = await fetch("https://roli.telkomsel.com/api/internal_counter/my_counter?" + `key=m0b1l3&token=${token}&version_number=3.0.6&reqparam=${reqparam}`).then(res => res.text())
